...
def evaluate_model(model, tokenizer, test_file: str, save_predictions: bool = True):
    ...
    for i, item in enumerate(test_data):
        ...
        # prediction = validate_and_fix_json(prediction)
...
def fix_json_format(text):
    """Post-process model output to fix JSON syntax errors"""
    import re
    ...
    # FIX THE MAIN ISSUE: Remove extra ]} at end
    text = re.sub(r'\]\}\]\}$', ']}', text)
...
if __name__ == "__main__":
    ...
    required_files = ["train.json", "test.json", "val.json"]
    ...
    predictions = evaluate_model(model, tokenizer, "test.json")
    metrics = calculate_accuracy_metrics(predictions)
